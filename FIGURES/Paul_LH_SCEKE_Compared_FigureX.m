%% Paul_LH_SCEKE_Compared_FigureX.m (version 1.1)
%Author: Paul Ernst
...
tic
%title of figure
titlestring = "Ernst-SCCompAvg-FigureX.tiff";
...
ye = 1:365;
%Annotations
anno = cell(1,4); anno{1,1} = "(a) Eddy Amplitude (median value, cm)";
...
annoy = cell(1,4); annoy{1,1} = 18.5; annoy{1,2} = 183;
annoy{1,3} =2700; annoy{1,4} = 1780;
...
for nTiles = 1:3
    ...
    grid on
    if nTiles == 1
       lgd = legend({'Full Years', 'Half Years', 'Local Years'}, 'FontSize', 28, 'Location', 'northeast');
       lgd.NumColumns = 3;
    end
end
...
%multiply to get to proper units if necessary
if ekeflag
